...
func (service *AuthService) RegisterUser(username, email, password string) (*domain.User, error) {
...
func (service *AuthService) GetUserFromToken(tokenString string) (*domain.User, error) {
	token, err := service.ValidateToken(tokenString)
	if err != nil {
		return nil, err
	}
